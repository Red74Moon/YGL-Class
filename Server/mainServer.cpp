...
int main()
{
	...
	// 연결위주로 사용한다.(SOCK_STREAM와 SOCK_DGRAM-(보내는 중심) 두가지가있다.)
	// 0 보통 0을 사용
	...
	serverAddr.sin_addr.s_addr = htonl(INADDR_ANY); // 4바이트 IP만들어 준다. ip 넣는 것
	serverAddr.sin_port = htons(9190); // port  (0~65535) 컴퓨터의 프로그램별 접속 번호

	// 3.ip, port binding
	//         사용하는 소켓            주소             사이즈
	if (bind(hServerSocket, (SOCKADDR*)&serverAddr, sizeof(serverAddr)) == SOCKET_ERROR) // 소켓(포트) 충돌, 이미사용중, 포트는 기본적으로 동시에 사용할 수 없다. 
	{
	...
	//{
		...
		send(hClientSocket, message, sizeof(message), 0); // 0  플레그 
